...
// remove just one app from all the permissions: tccutil reset All asia.redesk.app
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        ...
        s = s.replace("com.carriez.RustDesk_server", &format!("{}_server", bundleid));
    }
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/ReDesk.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another ReDesk.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // ReDesk.app/Contents/MacOS/RustDesk
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
